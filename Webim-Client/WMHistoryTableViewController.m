...
- (BOOL)canContinueChatInRealtimeSession:(WMSession *)session {
    ...
    return !(chat == nil || chat.state == WMChatStateUnknown || chat.state == WMChatStateClosed || !session.hasOnlineOperator);
}

- (IBAction)startChatButtonAction:(id)sender {
    if ([WebimController shared].realtimeSession.hasOnlineOperator) {
