...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return MAX(data.insets.top + data.view.frame.size.height + data.insets.bottom + 4, self.showAvatars ? 52 : 0);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        cell.frame = CGRectMake(0, 0, tableView.frame.size.width, [UIBubbleHeaderTableViewCell height]);
        cell.date = data.date;
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    NSBubbleData *data = [self bubbleTableViewBubbleAtIndexPath:indexPath];
    if (data == nil) {
        return;
    }
    if ([_bubbleDataSource respondsToSelector:@selector(tableView:accessoryButtonTappedForItem:)]) {
        [_bubbleDataSource tableView:self accessoryButtonTappedForItem:data.userData];
    }
}
